...
`node6` and `node14` are the same 48G hardware class → clean modded-vs-stock control.

## Headline results

| metric | 48G modded (node6) | 48G stock (node14) | 24G modded (node13) |
| --- | --- | --- | --- |
| `nvidia-smi topo -p2p` | **OK** all pairs | **CNS** all pairs | **OK** all pairs |
| `p2pBandwidthLatencyTest` P2P=Enabled | ~**26 GB/s** | ~2–3 GB/s (no P2P) | ~26 GB/s |
| `p2pcheck` 256 MiB, verified | 26.3 GB/s, 0 bad | noP2P | 26.3 GB/s, 0 bad |
| NCCL all_reduce busbw (`P2P_LEVEL=SYS`) | **24.8 GB/s** | 2.6 GB/s (SHM) | **25.1 GB/s** |
| NCCL all_reduce busbw (default) | 2.6 GB/s (SHM) | 2.6 GB/s | 4.6 GB/s (SHM) |
| >32 GB usable w/ P2P (cuMem, `20-…`) | **PASS** — 38 GiB resident + `via P2P/CUMEM`, correct | n/a | n/a (24G) |
| legacy `cudaDeviceEnablePeerAccess` cap (`21-…`) | **31.75 GiB** (BAR1) | n/a | n/a |

The stock node is the control: identical 48G cards, P2P reported **CNS** and NCCL stays on SHM
(~2.6 GB/s) — exactly what the mod removes.
